		...
		var handleDataTableFixedHeader = function() {
            ...
            if ($('#data-table-fixed-header').length !== 0) {
                var table=$('#data-table-fixed-header').DataTable({
                    ...
                    responsive: true,
                    ajax:"{{ url('pusatkendali/get_data')}}",
					columns: [
					...
					<div id="modal-tambah" class="modal fade" tabindex="-1" aria-hidden="true">
						<div class="modal-dialog modal-lg">
							<div class="modal-content">
								...
								<form id="mydata" method="post" action="{{ url('pusatkendali') }}" enctype="multipart/form-data">
									@csrf
									<!-- <input type="submit"> -->
...
@push('ajax')
	<script>
		function tambah(id){
			...
			$('#tampil-form').load("{{url('pusatkendali/modal')}}?id="+id);
		}
                    ...
                    success: function(msg){
                        ...
                        if(bat[1]=='ok'){
                            ...
                            var tables=$('#data-table-fixed-header').DataTable();
          							tables.ajax.url("{{ url('pusatkendali/get_data')}}").load();
